...
## Testing Guidelines
Place unit tests next to the code they cover with `#[cfg(test)]`. Add integration tests under `tests/` when behavior spans modules. Name tests by behavior, e.g. `loads_config_from_env` or `rejects_path_traversal`.  
