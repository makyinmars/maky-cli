...
- [ ] Step 1: Define `ToolHandler` trait and tool registry contract.
- [ ] Step 2: Define tool call/result event types for provider-session pipeline.
- [ ] Step 3: Add workspace-root canonicalization policy for path safety.
- [ ] Step 4: Add `list_files` architecture boundary and arguments schema.
- [ ] Step 5: Add `read_file` architecture boundary and arguments schema.
- [ ] Step 6: Add `exec_command` architecture boundary and arguments schema.
- [ ] Step 7: Define approval UI state and decision routing in controller.
- [ ] Step 8: Enforce approval requirement before risky tool execution.
- [ ] Step 9: Define stdout/stderr capture and output truncation policy.
- [ ] Step 10: Persist tool calls, approvals, and results in session events.
- [ ] Step 11: Define denial behavior and user-visible status messaging.

## Phase 6 Done Criteria (Checklist)

- [ ] At least one tool call path works end-to-end.
- [ ] Risky tool actions require explicit approval.
- [ ] Path traversal/symlink escape protections are defined.
- [ ] Tool outputs are displayed and persisted.
- [ ] Denied tool requests do not execute and are visible in UI/logs.
